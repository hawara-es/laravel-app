...
beforeEach(function () {
    $result = Blade::render('<x-auth.login-form />');
    $this->crawler = new Crawler($result);
    return $this->crawler->filter('form.auth-login');
});
...
test('The form action targets the login route', function () {
    $this->assertEquals(1, $this->crawler->filter('[action$="login"]')->count());
});
...
test('The form has an password field', function () {
